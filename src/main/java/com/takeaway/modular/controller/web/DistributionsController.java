...
@Api(value = "闪送配送管理", description = "DistributionsController")
public class DistributionsController {
	private static final Logger log = Logger
			.getLogger(DistributionsController.class);
	...
	@ApiOperation(value = "订单取消", httpMethod = "GET", notes = "取消闪送订单配送")
	@ApiImplicitParams({
		@ApiImplicitParam(name = "orderNo", value = "订单号", required = true, dataType = "String", paramType = "query")
		})
	...
	public JSONObject cancel(HttpServletRequest request,
		...
		try {
			...
			Map<String, Object> result=distributionsService.cancel(orders.getOrderNo(), orders.getIssorderno());
			return ErrorEnums.getResult(ErrorEnums.SUCCESS, "取消闪送订单配送", result);
		} catch (Exception e) {
			e.printStackTrace();
			return ErrorEnums.getResult(ErrorEnums.ERROR, "系统出现异常", null);
		}
